...
window.AdminPanel = ({ products, setProducts, setEditId, promocodes, setPromocodes }) => {
    ...
    const handleReset = async () => {
        ...
        if (hashHex !== TARGET_HASH) { alert("⛔️ Невірний пароль!"); return; }
        ...
        if (db) {
            ...
            if (window.INITIAL_PRODUCTS_SEED) {
    ...
    const handleResetTags = async () => {
        ...
        if (db) {
            const snapshot = await db.collection("products").get();
            const batch = db.batch();
            snapshot.docs.forEach((doc) => { batch.update(doc.ref, { isHit: false, isSale: false, rating: 5.0 }); });
            await batch.commit();
            alert("Теги скинуто!");
        } else {
    ...
    const handleDeleteAllPhotos = async () => {
         ...
         if (db) {
            const snapshot = await db.collection("products").get();
            const batch = db.batch();
            snapshot.docs.forEach((doc) => { batch.update(doc.ref, { images: [] }); });
            await batch.commit();
            alert("Фото видалено!");
         } else {
    ...
    const handleEdit = (product) => { setLocalEditId(product.id); setFormData({...product}); };

    const handleSave = async (e) => {
        e.preventDefault();
        if (!formData) return;
        try {
            if (db) await db.collection("products").doc(String(formData.id)).set(formData);
            else setProducts(products.map(p => p.id === formData.id ? formData : p));
            setLocalEditId(null);
        } catch (error) { console.error("Error saving:", error); alert("Помилка збереження!"); }
    };
    ...
    const handleDuplicate = async (product) => {
        ...
        if (db) await db.collection("products").doc(String(newProduct.id)).set(newProduct);
    ...
    const handleCreatePromo = async (e) => {
        e.preventDefault();
        const newPromo = { ...promoForm, id: Date.now().toString(), usedCount: 0, createdAt: new Date() };
        if (db) await db.collection("promocodes").doc(newPromo.id).set(newPromo);
        else setPromocodes([...promocodes, newPromo]);
                    ...
                    <form onSubmit={handleSave} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select value={formData.category} onChange={e=>setFormData({...formData, category:e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-violet-500">
                                    {(window.CATEGORIES || [{name:'Вібратори'}]).map(c=><option key={c.name} value={c.name}>{c.name}</option>)}
                        ...
                        <div className="bg-slate-900 p-4 rounded-xl border border-white/10">
                            <div className="flex justify-between items-center mb-4">
                                <label className="block text-sm font-bold text-gray-400">Медіа</label>
                                    ...
                                    <div key={i} className="relative w-32 h-32 rounded-lg overflow-hidden border border-white/20 group bg-black shadow-lg">
                                        ...
                                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition flex flex-col items-center justify-center gap-2">
                                            <div className="flex gap-2">
                                                <button type="button" onClick={() => moveImage(i, -1)} disabled={i === 0} className={`p-1 rounded bg-white/20 hover:bg-white/40 text-white transition ${i===0?'opacity-30':''}`}><window.Icons.ChevronLeft size={20}/></button>
                                                <button type="button" onClick={() => moveImage(i, 1)} disabled={i === formData.images.length-1} className={`p-1 rounded bg-white/20 hover:bg-white/40 text-white transition ${i===formData.images.length-1?'opacity-30':''}`}><window.Icons.ChevronRight size={20}/></button>
                                            </div>
                                            <button type="button" onClick={() => removeImage(i)} className="text-red-500 hover:scale-110 transition bg-white/20 p-1.5 rounded-full"><window.Icons.Trash2 size={20}/></button>
                                        </div>
                                    </div>
                                ))}
