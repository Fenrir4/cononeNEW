...
window.CartView = ({ cart, updateQty, removeFromCart, changeRoute, cartTotal, promocodes, applyPromo, appliedPromo, cancelPromo }) => {
    ...
    // Відправка замовлення в Firebase
    const handleOrderSubmit = async (e) => {
        ...
        try {
            if (window.firebase) {
                ...
                // Оновлюємо лічильник промокоду
                if (appliedPromo) {
                    db.collection('promocodes').doc(appliedPromo.id).update({
                        usedCount: firebase.firestore.FieldValue.increment(1)
                    });
                }
            } else {
                console.warn("Firebase не знайдено");
            }
            ...
            alert("Сталася помилка. Напишіть нам у підтримку.");
        } finally {
            setIsSending(false);
        }
    };
        ...
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6 animate-fade-in">
                ...
                <p className="text-gray-300 max-w-md mb-8 text-lg leading-relaxed">
                    Дякуємо за довіру! <br/> Менеджер скоро зв'яжеться з тобою для підтвердження.
                </p>
                <button onClick={() => { setIsSuccess(false); changeRoute('home'); }} className="bg-violet-600 hover:bg-violet-700 text-white px-8 py-4 rounded-full font-bold transition flex items-center gap-2">
                    На головну <i className="fa-solid fa-chevron-right"></i>
                </button>
            </div>
        );
    }
        ...
        return (
            <div className="text-center py-20 animate-fade-in">
                <div className="bg-slate-800 rounded-2xl border border-white/10 p-10 max-w-md mx-auto">
                    ...
                    <p className="text-gray-400 mb-6">Додайте щось цікавеньке, щоб розпалити вогонь 🔥</p>
                    <button onClick={() => changeRoute('home')} className="mt-4 text-violet-400 font-bold hover:text-white transition">Повернутися до покупок →</button>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-slate-900 py-8 px-4 animate-fade-in">
            <div className="max-w-4xl mx-auto">
                <button onClick={() => changeRoute('home')} className="mb-6 flex items-center gap-2 text-gray-400 hover:text-white transition group">
                    <i className="fa-solid fa-arrow-left group-hover:-translate-x-1 transition-transform"></i> Продовжити покупки
                </button>
                ...
                <div className="bg-slate-800 rounded-xl p-4 border border-white/10 mb-6 shadow-lg">
                    ...
                    ) : <p className="text-sm text-green-400 mb-2 font-bold flex items-center gap-2"><i className="fa-solid fa-fire"></i> Ура! У вас безкоштовна доставка!</p>}
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                            ...
                            {cart.map(item => (
                                <div key={item.id} className="p-4 flex gap-4 border-b border-white/5 items-center last:border-0">
                                    <div className="w-20 h-20 bg-slate-700 rounded-xl overflow-hidden flex-shrink-0">
                                        {item.images?.[0] ? <img src={item.images[0]} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-gray-500"><i className="fa-regular fa-image text-2xl"></i></div>}
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 bg-slate-900 rounded-lg p-1 border border-white/5">
                                        <button onClick={()=>updateQty(item.id, -1)} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white transition"><i className="fa-solid fa-minus"></i></button>
                                        <span className="w-4 text-center font-bold text-white text-sm">{item.qty}</span>
                                        <button onClick={()=>updateQty(item.id, 1)} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white transition"><i className="fa-solid fa-plus"></i></button>
                                    </div>
                                    <button onClick={()=>removeFromCart(item.id)} className="text-gray-500 hover:text-red-500 p-2 transition"><i className="fa-solid fa-trash"></i></button>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="bg-slate-800 rounded-xl p-4 border border-white/10 flex items-center gap-3">
                            <i className="fa-solid fa-ticket text-violet-500 text-xl"></i>
                            {appliedPromo ? (
                                <div className="flex-1 flex justify-between items-center">
                                    <span className="text-green-400 font-bold text-sm">Код {appliedPromo.code} застосовано!</span>
                                ...
                                <div className="flex-1 flex gap-2">
                                    <input 
                                        ...
                                        className="flex-1 bg-transparent text-white outline-none placeholder-gray-600 text-sm uppercase font-bold"
                                    />
                                    <button 
                                        onClick={()=> {const p = promocodes.find(c=>c.code===promoInput); if(p && p.usedCount < p.maxUses) applyPromo(promoInput); else alert("Невірний код або ліміт вичерпано"); setPromoInput("")}} 
                                        className="text-sm font-bold text-violet-400 hover:text-white transition"
                                    >
                                        ЗАСТОСУВАТИ
                                    </button>
                                </div>
                    ...
                    <div className="lg:col-span-1">
                        <form onSubmit={handleOrderSubmit} className="bg-slate-800 p-6 rounded-2xl border border-white/10 sticky top-24 shadow-2xl">
                            <h3 className="font-bold text-xl mb-6 text-white flex items-center gap-2">
                                <i className="fa-solid fa-bag-shopping text-violet-500"></i> Оформлення
                            </h3>
                            ...
                            <button disabled={isSending} className="w-full bg-violet-600 hover:bg-violet-700 text-white font-bold py-4 rounded-xl shadow-lg transition active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                                {isSending ? <i className="fa-solid fa-arrows-rotate fa-spin"></i> : "Підтвердити замовлення"}
                            </button>
                        </form>
                    </div>
                </div>
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1">
                        <form onSubmit={handleOrderSubmit} className="bg-slate-800 p-6 rounded-2xl border border-white/10 sticky top-24 shadow-2xl">
                            <h3 className="font-bold text-xl mb-6 text-white flex items-center gap-2">
                                <window.Icons.ShoppingBag className="text-violet-500" size={20}/> Оформлення
                            </h3>
    ...
    if (isSuccess) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6 animate-fade-in">
                ...
                <h2 className="text-3xl font-bold text-white mb-4">Замовлення прийнято! 🎉</h2>
                <p className="text-gray-300 max-w-md mb-8 text-lg leading-relaxed">
                    Дякуємо за довіру! <br/> Менеджер скоро зв'яжеться з тобою.
                </p>
                <button onClick={() => { setIsSuccess(false); changeRoute('home'); }} className="bg-violet-600 hover:bg-violet-700 text-white px-8 py-4 rounded-full font-bold transition flex items-center gap-2">
                    На головну <i className="fa-solid fa-arrow-right"></i>
                </button>
            </div>
        );
    }
    ...
    if (cart.length === 0) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6 animate-fade-in">
                ...
                <h2 className="text-2xl font-bold text-white mb-2">Кошик порожній</h2>
                <p className="text-gray-400 mb-8">Час додати трохи магії 🔥</p>
                <button onClick={() => changeRoute('home')} className="bg-violet-600 text-white px-8 py-3 rounded-full font-bold hover:bg-violet-500 transition">
                    В каталог
                </button>
            </div>
        );
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 pb-24 animate-fade-in">
            ...
            <h2 className="text-3xl font-bold mb-6 flex items-center gap-3">
                Твій кошик <span className="text-lg font-normal text-gray-500 bg-slate-800 px-3 py-1 rounded-full">{cart.reduce((a,b)=>a+b.qty,0)}</span>
            </h2>

            {/* --- БЛОК БЕЗКОШТОВНОЇ ДОСТАВКИ --- */}
            <div className="bg-slate-800 p-5 rounded-2xl border border-white/5 mb-8 shadow-lg">
                <div className="flex justify-between items-end mb-2">
                    <div className="text-sm font-medium text-gray-300">
                        {isFreeShipping ? (
                            <span className="text-green-400 flex items-center gap-1">✅ Безкоштовна доставка активована!</span>
                        ) : (
                            <span>До безкоштовної доставки ще <span className="text-white font-bold">{remainingForFreeShipping} ₴</span></span>
                        )}
                    </div>
                    <div className="text-xs text-gray-500">{Math.round(shippingProgress)}%</div>
                </div>
                <div className="w-full h-3 bg-slate-900 rounded-full overflow-hidden border border-white/5">
                    <div 
                        className={`h-full transition-all duration-700 ease-out ${isFreeShipping ? 'bg-gradient-to-r from-green-500 to-emerald-400' : 'bg-gradient-to-r from-violet-600 to-fuchsia-500'}`}
                        style={{ width: `${shippingProgress}%` }}
                    ></div>
                </div>
                {!isFreeShipping && (
                    <p className="text-xs text-gray-500 mt-2 text-center">Замов на 2000 ₴ — і доставка за наш рахунок 🔥</p>
                )}
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {cart.map(item => (
                        <div key={item.cartId} className="bg-slate-800 p-4 rounded-2xl flex gap-4 items-center border border-white/5 shadow-sm hover:border-violet-500/30 transition">
                            <div className="w-20 h-20 bg-slate-700 rounded-xl overflow-hidden flex-shrink-0 relative">
                                {item.images && item.images[0] ? <img src={item.images[0]} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xs">No img</div>}
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className="font-bold text-white truncate mb-1">{item.name}</h3>
                                <div className="text-violet-400 font-bold">{item.price} ₴</div>
                            </div>
                            <div className="flex items-center gap-3 bg-slate-900 rounded-lg p-1">
                                <button onClick={() => {
                                    const newQty = item.qty - 1;
                                    if (newQty > 0) setCart(cart.map(c => c.cartId === item.cartId ? {...c, qty: newQty} : c));
                                    else setCart(cart.filter(c => c.cartId !== item.cartId));
                                }} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white hover:bg-slate-700 rounded-md transition">-</button>
                                <span className="w-6 text-center font-bold text-sm">{item.qty}</span>
                                <button onClick={() => {
                                    setCart(cart.map(c => c.cartId === item.cartId ? {...c, qty: c.qty + 1} : c));
                                }} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white hover:bg-slate-700 rounded-md transition">+</button>
                            </div>
                        </div>
                    ))}
                    ...
                    {/* Промокод */}
                    <div className="bg-slate-800 p-5 rounded-2xl border border-white/5 mt-4">
                        <label className="text-xs text-gray-500 uppercase font-bold mb-2 block">Промокод</label>
                        <div className="flex gap-2">
                            <input 
                                type="text" 
                                placeholder="Введіть код..." 
                                className="flex-1 bg-slate-900 border border-slate-700 rounded-xl p-3 text-white uppercase placeholder:normal-case focus:border-violet-500 outline-none"
                                value={promoInput}
                                onChange={e => setPromoInput(e.target.value)}
                            />
                            <button onClick={handleApplyPromo} className="bg-slate-700 hover:bg-slate-600 text-white px-5 rounded-xl font-bold transition">ОК</button>
                        </div>
                        {appliedPromo && (
                            <div className="mt-3 text-green-400 text-sm flex items-center gap-2 bg-green-500/10 p-2 rounded-lg">
                                <i className="fa-solid fa-check"></i> 
                                Знижка <b>{appliedPromo.value}{appliedPromo.type==='percent'?'%':'₴'}</b> активована!
                                <button onClick={()=>setAppliedPromo(null)} className="ml-auto text-gray-400 hover:text-white text-xs underline">Скасувати</button>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="lg:col-span-1">
                    <form onSubmit={handleOrderSubmit} className="bg-slate-800 p-6 rounded-2xl border border-white/5 sticky top-24 shadow-xl">
                        <h3 className="font-bold text-xl mb-6 flex items-center gap-2">
                            <i className="fa-solid fa-bag-shopping text-violet-500"></i> Оформлення
                        </h3>
                        
                        <div className="space-y-4 mb-6">
                            <input required className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none" placeholder="Ім'я" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                            <input required type="tel" className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none" placeholder="Телефон (+380...)" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                            <input className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none" placeholder="Telegram (необов'язково)" value={formData.telegram} onChange={e => setFormData({...formData, telegram: e.target.value})} />
                            <input required className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none" placeholder="Місто" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} />
                            <input required className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none" placeholder="Відділення НП / Поштомат" value={formData.department} onChange={e => setFormData({...formData, department: e.target.value})} />
                            <textarea className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none h-20 resize-none text-sm" placeholder="Коментар" value={formData.comment} onChange={e => setFormData({...formData, comment: e.target.value})}></textarea>
                        </div>

                        <div className="space-y-3 mb-6">
                            <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition ${formData.payment === 'card' ? 'border-violet-500 bg-violet-500/10' : 'border-white/10 bg-slate-900 hover:bg-slate-700'}`}>
                                <input type="radio" name="payment" className="hidden" checked={formData.payment === 'card'} onChange={() => setFormData({...formData, payment: 'card'})} />
                                <div className="w-5 h-5 rounded-full border-2 border-gray-400 flex items-center justify-center">{formData.payment === 'card' && <div className="w-2.5 h-2.5 rounded-full bg-violet-500"></div>}</div>
                                <span className="font-medium">Оплата на карту</span>
                            </label>
                            <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition ${formData.payment === 'cod' ? 'border-violet-500 bg-violet-500/10' : 'border-white/10 bg-slate-900 hover:bg-slate-700'}`}>
                                <input type="radio" name="payment" className="hidden" checked={formData.payment === 'cod'} onChange={() => setFormData({...formData, payment: 'cod'})} />
                                <div className="w-5 h-5 rounded-full border-2 border-gray-400 flex items-center justify-center">{formData.payment === 'cod' && <div className="w-2.5 h-2.5 rounded-full bg-violet-500"></div>}</div>
                                <span className="font-medium">Післяплата</span>
                            </label>
                        </div>

                        <div className="bg-slate-900/50 p-4 rounded-xl space-y-2 text-sm mb-6">
                            <div className="flex justify-between text-gray-400"><span>Товари:</span><span>{cartTotal} ₴</span></div>
                            {discountAmount > 0 && <div className="flex justify-between text-green-400"><span>Знижка:</span><span>-{discountAmount} ₴</span></div>}
                            <div className="flex justify-between text-white font-bold text-lg pt-3 border-t border-white/5 mt-2"><span>До сплати:</span><span>{finalTotal} ₴</span></div>
                            {isFreeShipping && <div className="text-center text-green-400 text-xs mt-2 uppercase font-bold tracking-wider">✨ Безкоштовна доставка ✨</div>}
                        </div>

                        <button type="submit" disabled={isSending} className="w-full bg-violet-600 hover:bg-violet-700 text-white font-bold py-4 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 active:scale-95 disabled:opacity-70">
                            {isSending ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <i className="fa-solid fa-check"></i>}
    ...
    if (isSuccess) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6 animate-fade-in">
                ...
                <button onClick={() => { setIsSuccess(false); changeRoute('home'); }} className="bg-violet-600 hover:bg-violet-700 text-white px-8 py-4 rounded-full font-bold transition flex items-center gap-2">
                    На головну <window.Icons.ArrowRight size={18} />
                </button>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 pb-24 animate-fade-in">
            ...
            {/* --- БЛОК БЕЗКОШТОВНОЇ ДОСТАВКИ (НОВЕ) --- */}
            <div className="bg-slate-800 p-5 rounded-2xl border border-white/5 mb-8 shadow-lg">
                <div className="flex justify-between items-end mb-2">
                    <div className="text-sm font-medium text-gray-300">
                        {isFreeShipping ? (
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                    <form onSubmit={handleOrderSubmit} className="bg-slate-800 p-6 rounded-2xl border border-white/5 sticky top-24 shadow-xl">
                        <h3 className="font-bold text-xl mb-6 flex items-center gap-2">
                            <window.Icons.ShoppingBag size={20} className="text-violet-500" /> Оформлення
                        </h3>
        ...
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6 animate-fade-in">
                ...
                <p className="text-gray-300 max-w-md mb-8 text-lg leading-relaxed">
                    ...
                    Ми вже отримали твоє замовлення. Найближчим часом менеджер напише тобі в Telegram або зателефонує для підтвердження.
                </p>
                <div className="bg-slate-800 p-6 rounded-2xl border border-white/10 max-w-sm w-full mb-8 text-left">
                    <h3 className="text-white font-bold mb-3 flex items-center gap-2">
                        <window.Icons.Info size={18} className="text-violet-400"/> Що далі?
                    </h3>
                    <ul className="space-y-3 text-sm text-gray-400">
                        <li className="flex gap-2"><span className="text-green-500">✓</span> Ми перевіримо наявність товарів.</li>
                        <li className="flex gap-2"><span className="text-green-500">✓</span> Запакуємо в чорну непрозору плівку.</li>
                        <li className="flex gap-2"><span className="text-green-500">✓</span> Відправимо ТТН у SMS або Telegram.</li>
                    </ul>
                </div>
                <button onClick={() => { setIsSuccess(false); changeRoute('home'); }} className="bg-violet-600 hover:bg-violet-700 text-white px-8 py-4 rounded-full font-bold transition shadow-lg shadow-violet-500/30 flex items-center gap-2">
                    Повернутися на головну <window.Icons.ArrowRight size={18} />
                </button>
            </div>
        );
    }

    // ЕКРАН ПУСТОГО КОШИКА
    if (cart.length === 0) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6 animate-fade-in">
                <div className="text-6xl mb-6 opacity-50">🛒</div>
                <h2 className="text-2xl font-bold text-white mb-2">Кошик порожній</h2>
                <p className="text-gray-400 mb-8">Додай щось цікавеньке, щоб розпалити вогонь 🔥</p>
                <button onClick={() => changeRoute('home')} className="bg-violet-600 text-white px-8 py-3 rounded-full font-bold hover:bg-violet-500 transition">
                    В каталог
                </button>
            </div>
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 pb-24 animate-fade-in">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {cart.map(item => (
                        <div key={item.cartId} className="bg-slate-800 p-4 rounded-2xl flex gap-4 items-center border border-white/5 shadow-sm">
                            <div className="w-20 h-20 bg-slate-700 rounded-xl overflow-hidden flex-shrink-0">
                                {item.images && item.images[0] ? <img src={item.images[0]} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xs">No img</div>}
                            </div>
                            <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-3 bg-slate-900 rounded-lg p-1">
                                <button onClick={() => {
                                    const newQty = item.qty - 1;
                                    if (newQty > 0) {
                                        setCart(cart.map(c => c.cartId === item.cartId ? {...c, qty: newQty} : c));
                                    } else {
                    ...
                    {/* Блок промокоду */}
                    <div className="bg-slate-800 p-5 rounded-2xl border border-white/5 mt-4">
                        <label className="text-xs text-gray-500 uppercase font-bold mb-2 block">Маєш промокод?</label>
                        <div className="flex gap-2">
                            <input 
                                type="text" 
                                placeholder="Введіть код..." 
                ...
                <div className="lg:col-span-1">
                    <form onSubmit={handleOrderSubmit} className="bg-slate-800 p-6 rounded-2xl border border-white/5 sticky top-24 shadow-xl">
                        ...
                        <div className="space-y-4 mb-6">
                            <div>
                                <label className="text-xs text-gray-400 ml-1 mb-1 block">Ім'я *</label>
                                <input required className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none transition" 
                                    placeholder="Як до тебе звертатися?"
                                    value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                            </div>
                            <div>
                                <label className="text-xs text-gray-400 ml-1 mb-1 block">Телефон *</label>
                                <input required type="tel" className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none transition" 
                                    placeholder="+380..."
                                    value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                            </div>
                            <div>
                                <label className="text-xs text-gray-400 ml-1 mb-1 block">Нік в Telegram (необов'язково)</label>
                                <div className="relative">
                                    <span className="absolute left-3 top-3.5 text-gray-500">@</span>
                                    <input className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 pl-8 text-white focus:border-violet-500 outline-none transition" 
                                        placeholder="username"
                                        value={formData.telegram} onChange={e => setFormData({...formData, telegram: e.target.value})} />
                                </div>
                            </div>
                            <div>
                                <label className="text-xs text-gray-400 ml-1 mb-1 block">Місто *</label>
                                <input required className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none transition" 
                                    placeholder="Київ"
                                    value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} />
                            </div>
                            <div>
                                <label className="text-xs text-gray-400 ml-1 mb-1 block">Відділення НП / Поштомат *</label>
                                <input required className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none transition" 
                                    placeholder="Відділення №1"
                                    value={formData.department} onChange={e => setFormData({...formData, department: e.target.value})} />
                            </div>
                            <div>
                                <label className="text-xs text-gray-400 ml-1 mb-1 block">Коментар</label>
                                <textarea className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-white focus:border-violet-500 outline-none transition h-20 resize-none text-sm" 
                                    placeholder="Наприклад: 'Не дзвонити, пишіть в ТГ'"
                        ...
                        <div className="space-y-3 mb-6">
                            <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition ${formData.payment === 'card' ? 'border-violet-500 bg-violet-500/10' : 'border-white/10 bg-slate-900 hover:bg-slate-700'}`}>
                                <input type="radio" name="payment" className="hidden" 
                                    checked={formData.payment === 'card'} onChange={() => setFormData({...formData, payment: 'card'})} />
                            ...
                            <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition ${formData.payment === 'cod' ? 'border-violet-500 bg-violet-500/10' : 'border-white/10 bg-slate-900 hover:bg-slate-700'}`}>
                                <input type="radio" name="payment" className="hidden" 
                                    checked={formData.payment === 'cod'} onChange={() => setFormData({...formData, payment: 'cod'})} />
                                <div className="w-5 h-5 rounded-full border-2 border-gray-400 flex items-center justify-center">
                                    {formData.payment === 'cod' && <div className="w-2.5 h-2.5 rounded-full bg-violet-500"></div>}
                                </div>
                                <span className="font-medium">Післяплата (НП)</span>
                            </label>
                        </div>
                        ...
                        <div className="bg-slate-900/50 p-4 rounded-xl space-y-2 text-sm mb-6">
                            ...
                            <div className="flex justify-between text-white font-bold text-lg pt-3 border-t border-white/5 mt-2">
                                <span>До сплати:</span>
                                <span>{finalTotal} ₴</span>
                            </div>
                        </div>

                        <button type="submit" disabled={isSending} className="w-full bg-violet-600 hover:bg-violet-700 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-violet-900/50 flex items-center justify-center gap-2 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">
                            {isSending ? <window.Icons.Loader className="animate-spin" size={20}/> : <window.Icons.Check size={20}/>}
                            {isSending ? "Обробка..." : `Підтвердити замовлення`}
                        </button>
