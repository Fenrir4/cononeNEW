...
window.CartView = ({ cart, setCart, changeRoute, products, promocodes, appliedPromo, setAppliedPromo }) => {
    const { useState, useEffect } = React;
    ...
    if (appliedPromo) {
        if (appliedPromo.type === 'fixed') discountAmount = appliedPromo.value;
        else if (appliedPromo.type === 'percent') discountAmount = Math.round((cartTotal * appliedPromo.value) / 100);
    }
    ...
    const handleApplyPromo = () => {
        ...
        if (promo) {
            if (promo.maxUses && promo.usedCount >= promo.maxUses) {
                alert("Цей промокод вже вичерпано 😔");
                return;
            }
    ...
    const handleOrderSubmit = async (e) => {
        ...
        try {
            if (window.firebase) {
                ...
                if (appliedPromo) {
                    const promoRef = db.collection('promocodes').doc(appliedPromo.code);
                    await promoRef.update({
                        usedCount: firebase.firestore.FieldValue.increment(1)
                    }).catch(err => console.log("Promo update error:", err));
                }
            }
            setIsSuccess(true);
            setCart([]);
            ...
            alert("Помилка. Напишіть нам у Telegram.");
        } finally {
            setIsSending(false);
        }
    };
